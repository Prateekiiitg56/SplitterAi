    ...
    <Modal open={isOpen} onClose={onClose} title="Get SplitterAI Desktop & CLI">
      <div className="space-y-5 text-[var(--text)] py-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {platforms.map((p) => {
            ...
            return (
              <div
                key={p.id}
                className="p-3.5 rounded-panel border border-[var(--border)] bg-[var(--panel-2)] hover:border-[var(--accent)] transition-all flex flex-col justify-between group"
              >
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <Icon size={16} className="text-[var(--accent)]" />
                    <span className="font-semibold text-meta text-[var(--text)]">{p.name}</span>
                  </div>
                  <div className="text-micro font-mono text-[var(--faint)] mb-3">{p.arch}</div>
                </div>
        ...
        <div className="p-3.5 rounded-panel border border-[var(--border)] bg-[var(--panel)]">
          ...
          <div className="flex items-center justify-between bg-[var(--bg-inset)] border border-[var(--border)] rounded px-3 py-2 font-mono text-meta text-[var(--accent)]">
            <code>npm i -g @splitterai/cli</code>
            <button
              onClick={handleCopyCli}
              className="text-[var(--dim)] hover:text-[var(--text)] transition-colors flex items-center gap-1 text-micro font-sans"
