    ...
    <ProjectTabShell>
      <div className="flex-1 overflow-y-auto p-6 text-[var(--text)] space-y-6 max-w-[840px] mx-auto w-full select-none">
        ...
        <div className="flex items-center justify-between border-b border-[var(--border-soft)] pb-4">
          <div>
            <h2 className="text-title font-bold text-[var(--text)] tracking-tight flex items-center gap-2.5">
              <Users size={18} className="text-[var(--accent)]" />
              <span>Manual Agent Pipeline Builder</span>
            </h2>
            <p className="text-meta text-[var(--dim)] mt-0.5 font-mono">
          ...
          <button
            ...
            className={`btn-primary text-meta font-medium px-4 py-2 rounded-md border flex items-center gap-2 transition-all cursor-pointer ${
              !isFormValid
        ...
        <div className="space-y-1.5 border border-[var(--border-soft)] rounded-panel p-4 bg-[var(--panel)]">
          ...
          <input
            ...
            className="w-full bg-[var(--bg-inset)] border border-[var(--border)] rounded px-3 py-2 text-ui text-[var(--text)] font-medium focus:outline-none focus:border-[var(--accent)]"
          />
        </div>
        ...
        <div className="space-y-4">
          {drafts.map((agent, index) => {
            ...
            return (
              <div
                key={agent.id}
                className="border border-[var(--border-soft)] rounded-panel bg-[var(--panel)] p-5 space-y-4 hover:border-[var(--border)] transition-colors relative shadow-sm"
              >
                {/* Card Header Row */}
                <div className="flex items-center justify-between border-b border-[var(--border-soft)] pb-3">
                    ...
                    {editingLabelId === agent.id ? (
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          className="bg-[var(--bg-inset)] border border-[var(--accent)] rounded px-2 py-0.5 text-ui font-bold text-[var(--text)] focus:outline-none"
                          autoFocus
                        />
                        <button
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-strong font-bold text-[var(--text)] tracking-tight">
                          {agent.label}
                        </span>
                        <button
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-micro font-mono px-2.5 py-1 rounded bg-[var(--panel-2)] border border-[var(--border-soft)] text-[var(--dim)]">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {/* Role Selector */}
                  <div>
                    <label className="text-micro font-mono uppercase tracking-wider text-[var(--faint)] font-bold block mb-1">
                      AGENT ROLE
                    </label>
                    <select
                      value={agent.role}
                      ...
                      className="w-full bg-[var(--bg-inset)] border border-[var(--border)] rounded px-3 py-2 text-meta text-[var(--text)] font-medium cursor-pointer focus:outline-none focus:border-[var(--accent)]"
                    >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-[var(--bg-inset)] border border-[var(--border)] rounded p-2.5 text-meta text-[var(--text)] font-sans focus:outline-none focus:border-[var(--accent)] resize-none"
                  />
                </div>
              </div>
            )
        ...
        <div className="flex items-center justify-between pt-2">
          <button
            ...
            className="btn-secondary text-[var(--text)] font-medium text-meta px-4 py-2 rounded-md border border-[var(--border)] hover:border-[var(--accent)] flex items-center gap-2 transition-colors cursor-pointer"
          >
            <Plus size={14} />
            <span>+ Add agent</span>
          </button>
          ...
          <button
            onClick={handleLaunchPlan}
            disabled={!isFormValid}
            className={`btn-primary text-meta font-medium px-5 py-2.5 rounded-md border flex items-center gap-2 transition-all cursor-pointer ${
              !isFormValid
                ? 'opacity-50 cursor-not-allowed border-[var(--border-soft)] bg-[var(--panel-2)] text-[var(--faint)]'
                : 'border-[var(--border)] text-[var(--accent)] hover:border-[var(--accent)] shadow-md'
            }`}
